...
public class Drive extends Command {
    ...
    private final double LOWEST_SPEED = 0.1;
    ...
    protected void execute() {
    	...
    	if (parallelMove >= MOVE_DEADZONE || parallelMove <= -MOVE_DEADZONE) parallelMove = Math.signum(parallelMove) * 
    			(Math.pow(((Math.abs(parallelMove) - MOVE_DEADZONE) / (1 - MOVE_DEADZONE)), MOVE_EXPONENT) * (MOVE_SPEED - LOWEST_SPEED) + LOWEST_SPEED);
    	...
    	if (perpendicularMove >= MOVE_DEADZONE || perpendicularMove <= -MOVE_DEADZONE) perpendicularMove = Math.signum(perpendicularMove) * 
    			(Math.pow(((Math.abs(perpendicularMove) - MOVE_DEADZONE) / (1 - MOVE_DEADZONE)), MOVE_EXPONENT) * (MOVE_SPEED - LOWEST_SPEED) + LOWEST_SPEED);
    	else perpendicularMove = 0.0f;
    	
    	//Adjust Rotate
    	if (rotate >= ROTATE_DEADZONE || rotate <= -ROTATE_DEADZONE) rotate = Math.signum(rotate) * 
    			(Math.pow(((Math.abs(rotate) - ROTATE_DEADZONE) / (1 - ROTATE_DEADZONE)), ROTATE_EXPONENT) * (ROTATE_SPEED - LOWEST_SPEED) + LOWEST_SPEED);
    	else rotate = 0.0f;
    	
    	//Drive the Robot
    	Robot.driveTrain.drive(parallelMove, perpendicularMove, rotate, OI.robotOrientedDrive.get());
    }
